...
void RunBoardProbe() {
    ...
    ESP_LOGI(TAG, "board=fuji-waveshare-1p46 idf=%s", esp_get_idf_version());
    ...
    ESP_LOGI(TAG, "reset_reason=%s (%d)", ResetReasonName(reset_reason),
             static_cast<int>(reset_reason));

    if (flash_size != 16 * 1024 * 1024) {
        ESP_LOGW(TAG, "expected 16 MiB flash; verify the board SKU and build configuration");
    }
    if (psram_size != 8 * 1024 * 1024) {
        ESP_LOGW(TAG, "expected 8 MiB embedded PSRAM; verify Octal PSRAM configuration");
    }
}
